		...
		const fetchDashboardData = async () => {
			try {
				...
				if (departmentId) {
					...
					const servicesData = await fetchDepartmentServices(
					...
					if (servicesData.length > 0 && servicesData[0].department?.name) {
						...
						setDepartmentName(
							(user?.publicMetadata?.departmentName as string) ||
								"Your Department"
						);
					}
					...
					const activeServices = servicesData.filter(
						(s: GovernmentService) => s.status === "active"
					).length;
					setSummary({
					...
					const categoryDistribution =
						getServiceDistributionByCategory(servicesData);
					setServiceData(
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
					<Card>
						...
						<CardContent>
							...
							<p className="text-xs text-gray-500">
								...
								{summary.totalServices - summary.activeServices} inactive
							</p>
						</CardContent>
					</Card>
					...
					<Card>
						...
						<CardContent>
							...
							<p className="text-xs text-gray-500">
								{Math.round(
									(summary.activeServices / summary.totalServices) * 100
								)}
								% of total services
							</p>
						</CardContent>
