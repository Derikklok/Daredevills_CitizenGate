...
const fetchUserDepartment = async (clerkOrgId: string): Promise<Department | null> => {
  try {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const deptData: Department & { services: Service[] } = await deptRes.json();
        ...
        setServiceData(
          categoryDistribution.map((item, index) => ({
            ...item,
            color: ["#8884d8", "#82ca9d", "#ffc658", "#ff8042", "#0088FE", "#00C49F"][
              index % 6
            ],
          }))
        );
        ...
        if (!orgAppointmentsRes.ok) throw new Error("Failed to fetch org appointments");
        ...
        const deptAppointments = orgAppointments.filter(
          (appt) => appt.service?.department?.department_id === department.department_id
        );
        ...
        setSummary((prev) => ({
          ...prev,
          totalAppointments: deptAppointments.filter(a => a.appointment_status !== "draft").length,
      ...
      <div className="container mx-auto p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <Card className="hover:shadow-lg transition-shadow">
            ...
            <CardFooter>
              <Button className="w-full" onClick={() => navigate("/service-admin/services")}>
                View Services
              </Button>
            </CardFooter>
          </Card>
          ...
          <Card className="hover:shadow-lg transition-shadow">
            ...
            <CardFooter>
              <Button className="w-full" onClick={() => navigate("/service-admin/documents")}>
                Manage Documents
              </Button>
            </CardFooter>
          </Card>
