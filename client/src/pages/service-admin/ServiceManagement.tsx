...
// Enhanced types for service management
...
interface ExtendedGovernmentService extends GovernmentService {
	availability?: ServiceAvailabilityData;
}

type DepartmentWithAvailability = {
...
const fetchUserDepartment = async (
	orgId: string
): Promise<DepartmentWithAvailability | null> => {
	try {
		const res = await fetch(`${API_BASE_URL}/departments`);
		if (!res.ok) throw new Error("Failed to fetch departments");

		const departments: DepartmentWithAvailability[] = await res.json();
		const department =
...
const ServiceManagement = () => {
	...
	const [departmentServices, setDepartmentServices] = useState<
		ExtendedGovernmentService[]
	>([]);
	const [allServices, setAllServices] = useState<ExtendedGovernmentService[]>(
		[]
	);
	...
	const [editedService, setEditedService] =
		useState<ExtendedGovernmentService | null>(null);
	const [, setIsAddModalOpen] = useState<boolean>(false);
	const [, setNewService] = useState<{
		name: string;
		description: string;
		category: string;
	...
	useEffect(() => {
		const fetchServicesData = async () => {
			try {
				...
				const allServicesData = await fetchGovernmentServices();
				const allServicesWithStatus: ExtendedGovernmentService[] =
				...
				if (deptId) {
					const departmentServicesData = await fetchDepartmentServices(deptId);
					const departmentServicesWithStatus: ExtendedGovernmentService[] =
						departmentServicesData.map((s) => ({
							...s,
							status: s.status || "active",
						}));
					setDepartmentServices(departmentServicesWithStatus);
	...
	const handleViewDetails = (serviceId: string): void => {
		const service = [...departmentServices, ...allServices].find(
			(s) => s.service_id === serviceId
		);
		if (service) {
	...
	const handleSaveEdit = async (): Promise<void> => {
		...
		try {
			...
			const updatedService: GovernmentService = await response.json();

			// 2. Update/Create service availability if it exists
			if (editedService.availability) {
				const availabilityData = {
					service_id: updatedService.service_id,
					days_of_week: editedService.availability.days_of_week || [],
					start_time: editedService.availability.start_time || "09:00",
					end_time: editedService.availability.end_time || "17:00",
					duration_minutes: editedService.availability.duration_minutes || 30,
				};

				const availabilityResponse = await fetch(
					`${API_BASE_URL}/service-availability`,
					{
						method: "POST",
						headers: {
							"Content-Type": "application/json",
				...
				if (!availabilityResponse.ok) {
					console.warn(
						"Failed to update service availability, but service was updated"
					);
				} else {
			...
			// 3. Update state with the enhanced service
			const enhancedUpdatedService: ExtendedGovernmentService = {
				...updatedService,
				availability: editedService.availability,
			};

			setDepartmentServices((prev) =>
				prev.map((s) =>
					s.service_id === updatedService.service_id
						? enhancedUpdatedService
						: s
				)
			);
			setAllServices((prev) =>
