...
const AllGovernmentServices = () => {
  ...
  const departments = Array.from(
    new Set(services.map(service => JSON.stringify({ 
  ...
  const filteredServices = services.filter(service => {
    ...
    const matchesDepartment = selectedDepartment ? service.department.department_id === selectedDepartment : true;
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white shadow-sm py-4 px-6 mb-6">
        <div className="container mx-auto flex items-center justify-between">
          <h1 className="text-xl font-bold text-gray-800">CitizenGate</h1>
          <nav className="space-x-4">
            <Button variant="ghost" onClick={() => navigate("/")}>Home</Button>
            <Button variant="ghost" onClick={() => navigate("/booking-appointments")}>Book Appointment</Button>
            <Button variant="ghost" onClick={() => navigate("/my-appointments")}>My Appointments</Button>
            <Button variant="outline" className="bg-white">All Services</Button>
          </nav>
        </div>
      </header>
      ...
      <div className="container mx-auto p-6">
        <header className="mb-8">
          <h1 className="text-3xl font-bold text-gray-800">Government Services</h1>
          <p className="text-gray-600">Browse all services offered by the government</p>
        </header>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="mb-4 text-gray-600">
              ...
              {selectedCategory && <span> in <strong>{selectedCategory}</strong> category</span>}
              {selectedDepartment && <span> from <strong>
            ...
            {filteredServices.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredServices.map((service) => (
                  <Card key={service.service_id} className="overflow-hidden h-full flex flex-col">
                    ...
                    <CardContent className="pt-4 flex-1">
                      ...
                      <div className="mt-3 text-sm text-gray-500">
                        Estimated completion time: {service.estimated_total_completion_time}
                      </div>
                    </CardContent>
